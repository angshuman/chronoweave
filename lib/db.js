 ...
 *   - Also supports Vercel integration names: STORAGE_TURSO_DATABASE_URL / STORAGE_TURSO_AUTH_TOKEN
 *
...
function getClient() {
  ...
  // Support both plain names and Vercel integration prefixed names
  const tursoUrl = process.env.TURSO_DATABASE_URL || process.env.STORAGE_TURSO_DATABASE_URL;
  const tursoToken = process.env.TURSO_AUTH_TOKEN || process.env.STORAGE_TURSO_AUTH_TOKEN || "";
  ...
  if (tursoUrl) {
    ...
    console.log("[DB] Connected to Turso:", tursoUrl.slice(0, 40) + "...");
  } else if (process.env.VERCEL) {
    // Vercel without Turso -- use in-memory DB (ephemeral, for basic function testing)
    _db = createClient({ url: ":memory:" });
    console.warn("[DB] WARNING: No Turso URL found. Using in-memory DB (data will not persist).");
  } else {
...
async function initDb() {
  ...
  await db.batch([
    ...
    `CREATE TABLE IF NOT EXISTS likes (
      id TEXT PRIMARY KEY,
      user_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      published_id TEXT NOT NULL REFERENCES published_timelines(id) ON DELETE CASCADE,
      created_at TEXT DEFAULT (datetime('now'))
    )`,
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS idx_likes_unique ON likes(user_id, published_id)`,
    `CREATE INDEX IF NOT EXISTS idx_likes_published ON likes(published_id)`,
  ]);

  // -- Migrations: add columns if they don't exist yet --------------------
  // (Safe to run repeatedly -- each ALTER TABLE will fail silently if column exists)
  const migrations = [
    "ALTER TABLE published_timelines ADD COLUMN author_name TEXT DEFAULT ''",
    "ALTER TABLE published_timelines ADD COLUMN author_picture TEXT DEFAULT ''",
    "ALTER TABLE published_timelines ADD COLUMN like_count INTEGER DEFAULT 0",
    "ALTER TABLE published_timelines ADD COLUMN view_count INTEGER DEFAULT 0",
    "ALTER TABLE published_timelines ADD COLUMN description TEXT DEFAULT ''",
  ];
