...
function listSessions(userId) {
  ...
  let rows;
  if (userId) {
    rows = db.prepare("SELECT * FROM sessions WHERE user_id=? ORDER BY updated_at DESC").all(userId);
  } else {
    rows = db.prepare("SELECT * FROM sessions WHERE user_id IS NULL ORDER BY updated_at DESC").all();
  }
  return rows.map((r) => {
...
function createSession(name = "New Session", userId = null) {
  const db = getDb();
  const id = sid();
  db.prepare("INSERT INTO sessions (id, name, user_id) VALUES (?,?,?)").run(id, name, userId);
  return db.prepare("SELECT * FROM sessions WHERE id=?").get(id);
}

function deleteSession(sessionId, userId = null) {
  const db = getDb();
...
function updateSession(sessionId, name, userId = null) {
  const db = getDb();
  const session = db.prepare("SELECT * FROM sessions WHERE id=?").get(sessionId);
 ...
 *
 * If userId is provided, credits are checked and deducted.
 * If userId is null (anonymous/demo), no credit check.
 */
async function researchTopicStream({ session_id, query, name, color, userId }, onEvent) {
  ...
  if (userId) {
    if (!hasCredits(userId, operation)) {
      onEvent("error", {
    ...
    creditResult = deductCredits(userId, operation, session_id);
