...
/**
 * Streaming research -- LLM-first approach.
 *
 * Flow:
 * 1. Detect intent (research vs refine) via keyword matching — instant, no LLM call
 * 2. Check if web search is needed (only for recent/current/specific data requests)
 * 3. If search needed: do ONE quick search, then feed results to LLM
 * 4. If no search: go straight to LLM (fast path — the default)
 * 5. Stream LLM response, parse events, save to DB
 *
 * Types: status, intent, search_progress, search_complete, token, event_found, result, error
 */
async function researchTopicStream({ session_id, query, name, color }, onEvent) {
  ...
  // Step 1: Detect intent (instant — no LLM call)
  ...
  onEvent("intent", {
    ...
    summary: intent === "refine"
      ? "Modifying existing timeline"
      : "Researching new topic",
  });

  // Step 2: Check if web search would help (only for specific cases)
  ...
  if (wantsSearch && process.env.OPENAI_API_KEY) {
    ...
    try {
      ...
      // Continue without search — LLM handles it
    }
  }
  ...
  if (intent === "refine" && hasExisting) {
    ...
    // Research without search — the fast default path
    onEvent("status", {
      phase: "reasoning",
