 ...
 * Types: status, token, event_preview, result, error
 */
async function researchTopicStream({ session_id, query, name, color }, onEvent) {
  ...
  // Phase 2: Reasoning -- stream the raw LLM output
  onEvent("status", {
    phase: "reasoning",
  ...
  const prompt = RESEARCH_PROMPT.replace("{query}", query);
  let events;
  try {
  ...
  // Phase 3: Structuring -- save to DB
  onEvent("status", {
    phase: "structuring",
