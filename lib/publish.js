...
async function publishTimeline(userId, sessionId, title) {
  ...
  if (existing) {
    // Update
    await run(
      "UPDATE published_timelines SET title=?, data=?, description=?, author_name=?, author_picture=?, updated_at=datetime('now') WHERE id=?",
      [title || existing.title, JSON.stringify(data), desc, authorName, authorPicture, existing.id]
    );
    return {
      id: existing.id,
      slug: existing.slug,
  ...
  await run(
    ...
    [id, userId, sessionId, title || session.name, slug, JSON.stringify(data), desc, authorName, authorPicture]
  );
