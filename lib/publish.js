 ...
 * Prefers the session name (which is now a holistic LLM-generated title
 * reflecting all topics), with date range appended if missing.
 */
function generateTitle(data, sessionName) {
  ...
  // Primary source: session name (holistic title from LLM-powered updates)
  if (sessionName && sessionName !== "New Session" && !sessionName.startsWith("Thread")) {
    let base = sessionName;
