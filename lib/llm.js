/**
 * ChronoWeave — LLM helpers (multi-provider: Anthropic or OpenAI)
 *
 * Provider auto-detection:
 *   1. ANTHROPIC_API_KEY set → Anthropic (Claude)
 *   2. OPENAI_API_KEY set   → OpenAI (GPT-4o)
 *
 * In the Perplexity sandbox, api_credentials inject both keys and the
 * Anthropic SDK is used by default. Locally, set whichever key you have.
 */
...
async function llmCallOpenAI(prompt, maxTokens = 6000) {
  ...
  // Detect sandbox vs real OpenAI by checking the base URL
  // The sandbox proxy uses the Responses API; real OpenAI uses Chat Completions
  const baseUrl = process.env.OPENAI_BASE_URL || client.baseURL || "";
  const isSandbox = baseUrl.includes("proxy") || baseUrl.includes("pplx");

  if (isSandbox) {
    // Sandbox: Responses API
